...
(#Achraf Chellaoui# et #Clément Persicot#)
...
Oui le code est structuré en couche car il y une "Class", un "Controller", une "Application" et un "Repository".

Le "Controller" ne communique pas avec les autres composants.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier "RG1" se trouve dans le "Controller".
...
docker run : Permet d'executer un conteneur Docker.
-it : Option qui permet de démarrer le contenu en mode interactif et de connecter le temrinal de l'utilisateur à l'entrée standard du conteneur.
-e POSTGRES_PASSWORD=password : Permet de spécifier le mot de passe de l'utilisateur par défaut pour la BDD.
-p 5432:5432 : définit le mappage des ports entre le conteneur et l'hôte.
postgres : nom de l'image Docker à partir de laquelle le conteneur est créé.
...
- L'utilisation de ports et d'adaptateurs
- Le modèle de domaine est au cœur de l'application 
- Les événements sont utilisés pour communiquer entre les domaines
- L'application est construite autour de cas d'utilisation
